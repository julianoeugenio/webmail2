...
if (!empty($_POST['caixa']) && !empty($_POST['uid']) && !empty($_POST['id'])) {
    $caixa = $_POST['caixa'];
...
function Carrega_email_banco($id, $caixa, $uid) {
    ...
    if ($query) {
        if ($reg = ibase_fetch_assoc($query, IBASE_TEXT)) {
            ...
            $de = Utf8($reg['DE']);
            $para = Utf8($reg['PARA']);
            $copia = Utf8($reg['COPIA']);
            $copia_oculta = Utf8($reg['COPIAOCULTA']);
            
            $ctr_copia = '';
            
            if(!empty($copia)){
              $ctr_copia .= '<b style="color: #666;"> Copia: </b>'.$copia.'<br>';  
            }
            
            if(!empty($copia_oculta)){
              $ctr_copia .= '<b style="color: #666;"> Copia Oculta: </b>'.$copia_oculta.'<br>';  
            }
            ...
            $ret_ec = '<div id="div_msg_header" '
                    ...
                    . '<div style="float: left;"><b style="color: #666;"> De: </b>' . $de . '<br /><b style="color: #666;"> Para: </b>' . $para . '<br />'.$ctr_copia.'<b style="color: #666;"> Assunto: </b>' . $assunto . '<br /><b style="color: #666;"> Data: </b>' . $data . '</div>';
